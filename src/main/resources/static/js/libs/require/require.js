...
var requirejs, require, define; (function(ca) {
	...
	function ha(b) {
		function c(a, e, b) {
			...
			if (a && "." === a.charAt(0)) if (e) {
				...
				n = a = n.concat(a);
				d = n.length;
				for (e = 0; e < d; e++) if (c = n[e], "." === c) n.splice(e, 1),
				e -= 1;
		...
		function g(a) {
			var e = j(l.paths, a);
			if (e && H(e) && 1 < e.length) return e.shift(),
			i.require.undef(a),
			i.require([a]),
			!0
		}
		...
		function m(a, e, b, f) {
			...
			a = a[1];
			g && (g = c(g, h, f), d = j(p, g));
			a && (g ? k = d && d.normalize ? d.normalize(a,
			function(a) {
				return c(a, h, f)
		...
		function w(a, e) {
			...
			function(e) {
				if (e = j(k, e)) e.error = a,
				e.events.error && (f = !0, e.emit("error", a))
			}), !f) h.onError(a)
		}
		function x() {
		...
		function F(a, e, b) {
			var f = a.map.id;
			a.error ? a.emit("error", a.error) : (e[f] = !0, v(a.depMaps,
			function(f, c) {
				var d = f.id,
				g = j(k, d);
		...
		function K(a) {
			...
			e = i.onScriptError; (!a.detachEvent || Z) && a.removeEventListener("error", e, !1);
			return {
				node: a,
		...
		$.prototype = {
			init: function(a, e, b, f) {
				...
				if (!this.inited) {
					...
					else this.events.error && (b = t(this,
					function(a) {
						this.emit("error", a)
					}));
			...
			callPlugin: function() {
				...
				function(f) {
					...
					if (this.map.unnormalized) {
						if (f.normalize && (J = f.normalize(J,
						function(a) {
							return c(a, u, !0)
						}) || ""), f = m(a.prefix + "!" + J, this.map.parentMap), r(f, "defined", t(this,
						function(a) {
							this.init([],
							function() {
								return a
							...
							this.depMaps.push(f);
							if (this.events.error) g.on("error", t(this,
							function(a) {
								this.emit("error", a)
							}));
							g.enable()
						}
					} else g ? (this.map.url = i.nameToUrl(g), this.load()) : (d = t(this,
					function(a) {
						this.init([],
						function() {
							return a
			...
			enable: function() {
				...
				function(a, b) {
					...
					c = a.id;
					f = k[c]; ! s(N, c) && (f && !f.enabled) && i.enable(a, this)
				}));
		...
		i = {
			...
			configure: function(a) {
				...
				B(k,
				function(a, b) { ! a.inited && !a.map.unnormalized && (a.map = m(b))
				});
	...
	if ("undefined" === typeof define) {
		...
		h = requirejs = function(b, c, d, g) {
			...
			u && u.context && (m = u.context); (g = j(F, m)) || (g = F[m] = h.s.newContext(m));
			u && g.configure(u);
		...
		h.load = function(b, c, d) {
			...
			g.attachEvent && !(g.attachEvent.toString && 0 > g.attachEvent.toString().indexOf("[native code")) && !Z ? (O = !0, g.attachEvent("onreadystatechange", b.onScriptLoad)) : (g.addEventListener("load", b.onScriptLoad, !1), g.addEventListener("error", b.onScriptError, !1)),
			g.src = d,
			L = g,
		...
		function(b) {
			y || (y = b.parentNode);
			if (K = b.getAttribute("data-main")) return q = K,
		...
		define = function(b, c, d) {
			...
			H(c) || (d = c, c = null); ! c && G(d) && (c = [], d.length && (d.toString().replace(la, "").replace(ma,
			function(b, d) {
				c.push(d)
			}), c = (1 === d.length ? ["require"] : ["require", "exports", "module"]).concat(c)));
			if (O) {
				if (! (g = L)) P && "interactive" === P.readyState || U(document.getElementsByTagName("script"),
				function(b) {

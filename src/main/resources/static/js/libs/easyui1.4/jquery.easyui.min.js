...
(function($) {
	...
	function _af(_b0, _b1) {
		...
		if (_b3.total == 0) {
//			_b3.pageNumber = 0;   TODO 防止初始表格时无数据会重复请求的问题
			_b4 = 0;
		}
		if (bb.num) {
...
(function($) {
	...
	$.extend($.fn.validatebox.defaults.rules, {
		md : {
			validator : function(value) {
				return /^(\d{4})-(0\d{1}|1[0-2])-(0\d{1}|[12]\d{1}|3[01])$/
						.test(value);
			},
			message : 'Please enter a valid date {0}.'
		},
		mdt : {
			validator : function(value) {
		...
		compmd : {
        	validator: function(value, param) {
        		var compvalue = $("#" + param[0]).datetimebox('getValue');
        		var d1 = null;
        		if(compvalue){
        			d1 = $.fn.datebox.defaults.parser(compvalue);
        		}
				var d2 = $.fn.datebox.defaults.parser(value);

				if (d1 != null && d2 != null) {
					var start = new Date(value.replace("-", "/").replace("-", "/")); 
        			var end = new Date(compvalue.replace("-", "/").replace("-", "/")); 
        			
        			if (param[1] == "ismin") {
        				return start < end;
   				 	}
        			if (param[1] == "isminEqual") {
        				return start <= end;  
                    }
        			if (param[1] == "ismax") {
        				return start > end;
                    }
        			if (param[1] == "ismaxEqual") {
        				return start >= end;  
                    }
        		}else{
        			return true;
	...
	$.extend($.fn.validatebox.defaults.rules, {
		maxLength : {
			...
			message : 'length can not be above {0} characters.'
		},
		minLength : {
			validator : function(value, param) {
				return param[0] <= value.length;
			},
			message : 'Length cannot be less than {0} characters.'
		},
		CHS : {
			validator : function(value) {
				return /^([\u0391-\uFFE5])*$/.test(value);
			},
			message : "The input Chinese characters"
		},
		mobile : {     
            validator: function(value){     
                return /^1[3|4|5|7|8]\d{9}$/i.test($.trim(value));     
            },     
            message: 'Mobile phone number format error'    
        },
        landline : {     
            validator: function(value){     
                return /^(^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})))*$/.test($.trim(value));     
            },     
            message: 'landline number format error'    
        },
        compnum:{
        	validator: function(value, param) {
        		var compvalue = $("#" + param[0]).val();
        		if (value != null) {
        			if (compvalue) {
        				 if (param[1] == "ismin") {
        					 if (value*1 > 0 || compvalue*1 > 0) {
                                 if (value*1 < compvalue*1) {
                                     return true;
                                 }
                                 else {
                                     return false;
